...
本プロジェクトは ACM プラグインを使用している。以下のルールに従うこと。

### Corrective Instruction の報告

セッション中にユーザーから修正指示を受けた場合（アプローチ変更、やり直し要求、不満表明、方向転換）、
`acm_record_signal` MCP ツールを呼び出して記録する:

- `session_id`: 現在のセッション ID（session-start hook で注入される）
- `event_type`: `"corrective_instruction"`
- `data`: `'{"prompt":"<ユーザーメッセージ抜粋>","reason":"<簡潔な説明>"}'`
