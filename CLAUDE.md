...
本プロジェクトは研究プロジェクトである。意見を返す際には忖度をせず、誠実かつ正直に返答すること。
ユーザーの提案であっても技術的に問題がある場合は率直に指摘し、根拠を示すこと。
...
4. **GitHub Flow**: `main` / `feature/*`（develop不使用）
...
## ACM (Associative Context Memory) の利用

本プロジェクトは ACM プラグインを使用している。

### Corrective Instruction の検出

Corrective instruction（ユーザーの修正指示）は session-end 時に transcript JSONL を
自動解析して検出される。手動での報告は不要。

検出方式:
1. **Primary**: Ollama ローカル LLM による分類（`gemma2:2b`, `temperature: 0`）
2. **Fallback**: Ollama 不可時は interrupt 直後メッセージの構造検出のみ

詳細は `docs/SPECIFICATION.md` Section 3.6 を参照。
