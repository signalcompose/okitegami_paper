...
- `Embedder.initialize()` is bounded by `embedder_init_timeout_ms` (default `30000`; userConfig `CLAUDE_PLUGIN_OPTION_EMBEDDER_INIT_TIMEOUT_MS`, valid `0`–`120000`). 30 s covers first-run model download over slow networks while still preventing indefinite hangs. On timeout, entries are persisted without embedding (existing fallback path).
- The Phase 2 entry loop is bounded by `pre_compact_budget_ms` (default `30000`; userConfig `CLAUDE_PLUGIN_OPTION_PRE_COMPACT_BUDGET_MS`, valid `0`–`300000`). When exceeded, remaining entries are skipped; boundary advancement follows the same partial/no-advance rules (`persisted === 0` → do not advance, else advance). A `budget_exceeded` flag is recorded in the `pre_compact_experiences_created` log event.
