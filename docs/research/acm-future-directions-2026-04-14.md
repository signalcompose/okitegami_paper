...
| **MINJA** (2026) | arxiv:2604.02623 | クエリのみでメモリ汚染 | embedding 検索の操作リスク |
...
### 6.3 SessionEnd vs Stop — 長セッションの問題
...
### 6.4 UserPromptSubmit によるリアルタイム corrective detection [改善推奨]
...
2. [6.6]   verbosity 設定 +        ← ユーザーへの ACM 動作表示
           systemMessage 可視化
3. [6.6]   JSONL ログ永続化         ← 運用診断基盤
4. [6.3]   SessionEnd + PreCompact  ← Stop からの移行、長セッション対応
           hook ペア運用
...
8. [6.4]   UserPromptSubmit         ← リアルタイム corrective detection
           リアルタイム検出
9. [6.5]   plugin hooks/skills      ← plugin system 最新化
           への移行
```
