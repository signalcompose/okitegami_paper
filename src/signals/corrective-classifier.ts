...
function buildClassificationPrompt(messages: Array<{ index: number; text: string }>): string {
...
export async function isOllamaAvailable(url: string): Promise<boolean> {
  try {
    ...
    if (!(err instanceof Error && err.name === "AbortError")) {
      console.error(
        `[ACM] isOllamaAvailable: probe to "${url}/api/tags" failed: ` +
...
async function classifyWithOllama(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[ACM] classifyWithOllama: Ollama response is not valid JSON: ${data.response.slice(0, 100)}`
      );
      return null;
    }
    ...
    return parsed as OllamaClassification[];
  } catch (err) {
    if (err instanceof Error && err.name === "AbortError") {
      console.error(`[ACM] classifyWithOllama: request timed out after ${config.timeoutMs}ms`);
    } else {
      console.error(
        `[ACM] classifyWithOllama: unexpected error: ${err instanceof Error ? err.message : String(err)}`
      );
    }
    return null;
  } finally {
...
function structuralFallback(transcript: ParsedTranscript): ClassifiedMessage[] {
  const results: ClassifiedMessage[] = [];

  for (const turn of transcript.turns) {
    if (turn.index === 0) continue;
    if (!turn.isAfterInterrupt) continue;
    ...
    if (CONFIRMATION_PATTERNS.some((p) => p.test(text))) continue;
...
export async function classifyCorrections(
  transcript: ParsedTranscript,
  config?: ClassifierConfig
): Promise<ClassifiedMessage[]> {
  if (transcript.turns.length <= 1) return [];

  const resolvedConfig: Required<ClassifierConfig> = {
  ...
  if (!ollamaUp) {
    console.error(
      `[ACM] classifyCorrections: Ollama unavailable at "${resolvedConfig.ollamaUrl}", ` +
  ...
  if (messagesToClassify.length === 0) return [];
  ...
  if (!llmResults) {
    console.error(
      "[ACM] classifyCorrections: LLM classification failed, using structural fallback"
    );
    return structuralFallback(transcript);
  }
  ...
  for (const classification of llmResults) {
    ...
    if (!classification.corrective) continue;
